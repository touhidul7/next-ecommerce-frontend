 ...
 *  sku (optional),
 *  variant_id (optional - legacy support)
 * }
 */
function normalizeIncomingItem(payload) {
  ...
  // Handle both camelCase and snake_case for SKU
  const sku = payload?.sku ?? payload?.SKU ?? null;
  ...
  return {
    ...
    sku: sku,  // ✅ Save SKU
    // Also save these explicitly to ensure they're stored
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "ADD_ITEM": {
      ...
      // If missing critical pricing, still add but keep price null (UI can handle)
      ...
      if (idx >= 0) {
        ...
        updated[idx] = { 
          ...
          // Update SKU and variantId in case they changed
...
export function CartProvider({ children }) {
  ...
  const api = useMemo(() => {
    return {
      ...
      updateItem: (lineId, updates) => dispatch({ type: "UPDATE_ITEM", payload: { lineId, updates } }),
