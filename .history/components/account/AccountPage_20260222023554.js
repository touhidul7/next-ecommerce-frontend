...
useEffect(() => {
  ...
  authFetch("/api/customer/orders")
    .then((data) => {
      ...
      // Fix: Handle the paginated response structure correctly
      console.log("Orders response:", data); // Add this for debugging
      
      if (data.success && data.orders) {
        // Your backend returns { success: true, orders: { data: [...] } }
        const ordersData = data.orders.data || [];
        setOrders(ordersData);
      } else {
        setOrders([]);
        setOrdersError("Invalid response format");
      }
    })
    .catch((e) => {
      if (!alive) return;
