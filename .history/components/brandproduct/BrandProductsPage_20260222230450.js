...
function isVariableProduct(p, variants) {
  ...
  // sometimes backend uses 2 for variable, 1 for simple etc.
  if (p?.product_type_id === 2) return true;
  return false;
}

function rangeForVariable(variants) {
  const vals = variants
    .map((v) => pickPrice(v))
    .filter((x) => x != null);
  ...
  const fetchBrandProducts = useCallback(
    async (isRetryAttempt = false) => {
      ...
      try {
        ...
        // ✅ Handle many possible response shapes:
        // [] OR {products: []} OR {data:{products:[]}} OR {data:[]}
        const list =
...
function BrandProductCard({ p, baseUrl, showAddedFeedback, onAddToCart }) {
  ...
  const priceNode = useMemo(() => {
    ...
    const rg = rangeForVariable(variants);
    return (
      <div className="mt-2 font-extrabold">
  ...
  return (
    <div className="bg-white rounded-2xl soft-card overflow-hidden h-full flex flex-col">
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <pre className="mt-3 text-[10px] text-slate-500 whitespace-pre-wrap">
          type: {String(p?.product_type ?? p?.type)} | variants: {variants.length}
        </pre>
        */}
      </div>
    </div>
