...
import { ShoppingCart, SlidersHorizontal, X, RotateCw, ArrowRight, Heart } from "lucide-react";
...
export default function BrandProductsPage({ slug = "special-offer", title }) {
  ...
  const SortDropdown = () => (
    <select
      ...
      className="rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#008159]"
    >
          ...
          <div className="h-44 flex items-center justify-center">
            {imgSrc ? (
              <img
                src={imgSrc}
                alt={p.name}
                className="object-contain w-50 h-50 rounded-xl"
                loading="lazy"
              />
            ) : (
