...
export default function Header() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      // ✅ Option A (recommended): if you have a Next.js proxy route:
  ...
  const siteName = settings?.software_name || "";
  const locationText = (() => {
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 z-50 animate-fadeIn">
                  <div className="py-2">
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span><Heart className="w-4 h-4" /></span> Wishlist
                      </span>
                    </Link>
