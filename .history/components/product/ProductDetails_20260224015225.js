...
import { Heart, ShoppingCart, RotateCw, MessageCircleQuestionMark, MessageCircleMore } from "lucide-react";
...
export default function ProductDetails({ productId }) {
  ...
  function BuyNowButton({ p, selectedVariant, isVariable, baseUrl }) {
    ...
    // ✅ If variable product, variant MUST be selected
    if (isVariable && !selectedVariant?.id) {
    ...
    const oldPrice = isVariable
      ? (v?.regular_price && v?.sale_price ? Number(v.regular_price) : null)
      : (p?.regular_price && p?.sale_price ? Number(p.regular_price) : null);
    ...
    // ✅ Build a readable variant label from attributes
    const variantLabel =
      isVariable && v?.attributes && typeof v.attributes === "object"
    ...
    const itemPayload = {
      productId: p?.id,
      product_id: p?.id, // ✅ extra safety for checkout mapping
      variantId: v?.id ?? null,
      variant_id: v?.id ?? null, // ✅ extra safety for backend + other pages
      name: p?.name ?? "",
      variantLabel, // ✅ show in checkout UI
      image,
      price,
      oldPrice,
      stock: isVariable ? (v?.stock ?? null) : (p?.stock ?? null),
      attrs: isVariable ? (v?.attributes ?? null) : null,
      sku: isVariable ? (v?.sku ?? null) : (p?.sku ?? null), // ✅ keep sku
    };

    addItem(itemPayload, 1);
