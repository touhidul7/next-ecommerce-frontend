...
export default function CartPage() {
  ...
  // Calculate shipping based on product shipping_price
  const shipping = useMemo(() => {
    if (items.length === 0) return 0;
    
    // Sum up shipping from all products
    const totalShipping = items.reduce((sum, it) => {
      const shippingPrice = Number(it.shipping_price) || 0;
  ...
  const onApplyCoupon = async () => {
    ...
    try {
      console.log("Applying coupon:", code, "with amount:", subtotal);
      ...
      if (!response.ok) {
        ...
        console.error("Error response:", text.substring(0, 200)); // Log first 200 chars
      ...
      if (text.trim().startsWith('<!DOCTYPE') || text.trim().startsWith('<html')) {
        ...
        setCouponError("API endpoint not found. Please check the URL.");
        return;
      }
      ...
      if (data.success) {
        // Apply coupon with the discount amount from API response
        applyCoupon({
          code: data.data.code,
      ...
      setCouponError(`Failed to apply coupon: ${error.message}. Please check the API URL and try again.`);
    } finally {
          ...
          <div className="flex items-center gap-4">
            {shippingPrice > 0 && (
              <span className="text-xs text-slate-500">
                Shipping: {formatBDT(shippingPrice)}
              </span>
            )}
