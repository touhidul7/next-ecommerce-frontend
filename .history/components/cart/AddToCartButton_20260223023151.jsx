    ...
    <div className="bg-slate-50 min-h-screen">
      <Container className="py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          {/* Button to show cart popup */}
          <button
            onClick={() => setShowCartPopup(true)}
            className="rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-semibold hover:bg-slate-50 w-fit transition-colors flex items-center gap-2"
          >
            <span>🛒</span>
...
function CartPopup({ items, subtotal, shipping, discount, total, appliedCoupon, formatBDT, onClose }) {
  return (
    <>
      {/* Backdrop */}
      <div 
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl z-50 w-full max-w-md max-h-[80vh] overflow-y-auto">
        ...
        <div className="p-4">
          ...
          <div className="mt-6">
            <button
              onClick={onClose}
              className="w-full rounded-xl bg-emerald-600 text-white px-4 py-3 font-semibold hover:bg-emerald-700 transition-colors"
            >
              Continue Checkout
            </button>
          </div>
