...
export default function CheckoutPage() {
  ...
  const createOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        tax_rate_id: null, // Set if you have tax rates
        shipping: shipping,
      ...
      return data.order || data.data; // Adjust based on your API response structure
    } catch (error) {
  ...
  const updatePayment = async (orderId) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/orders/${orderId}/payment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      // Step 2: Update payment (if not COD)
      if (payment !== "cod") {
                ...
                {payment !== "cod" && (
                  <div className="mt-4 rounded-xl border border-gray-300 bg-white p-4">
                    ...
                    <div className="mt-3 p-3 bg-amber-50 rounded-lg text-sm">
                      <span className="font-semibold">Amount to pay: {formatBDT(total)}</span>
