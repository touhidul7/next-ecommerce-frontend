  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        // In your placeOrder function, update the items mapping:

items: items.map((item) => ({
  product_id: item.productId || item.product_id || null,
  product_name: item.name,
  sku: item.sku || null,  // ✅ This will now work
  qty: Number(item.qty || 1),
  price: Number(item.price || 0),
  variant_id: item.variantId ?? item.variant_id ?? null  // ✅ This will now work
})),
