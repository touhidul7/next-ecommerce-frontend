...
function getVariantLabel(item) {
  // API returns item.variant (object) OR maybe item.variant_name
  const v = item?.variant;

  if (v) {
    // choose best available label
    const label = v.name || v.value || v.title || "";
    const sku = v.sku ? ` • ${v.sku}` : "";
    return label ? `${label}${sku}` : (v.sku ? `SKU: ${v.sku}` : "");
  }
            ...
            <div className="border border-gray-300 bg-white rounded-2xl p-6 soft-card">
              ...
              <div className="mt-5 grid md:grid-cols-2 gap-4">
                <InfoBox
                  ...
                  value={`Shipping Address: ${order.shipping_address}\Billing Address: ${order.billing_address}\Customer Phone: ${order.customer?.phone || 'N/A'}`}
                />
                <InfoBox
                        ...
                        <div>
                          ...
                          {getVariantLabel(item) && (
                            <div className="text-xs text-slate-600 mt-1">
                              Variant: <span className="font-semibold">{getVariantLabel(item)}</span>
                            </div>
                          )}
