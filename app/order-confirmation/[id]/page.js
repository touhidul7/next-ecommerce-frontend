            ...
            <div className="border border-gray-300 bg-white rounded-2xl p-6 soft-card">
              ...
              <div className="mt-5 grid md:grid-cols-2 gap-4">
                <InfoBox
                  ...
                  value={`Shipping Address: ${order.shipping_address}\nBilling Address: ${order.billing_address}`}
                />
                <InfoBox
            ...
            <div className="border border-gray-300 bg-white rounded-2xl soft-card overflow-hidden">
              ...
              <div className="divide-y divide-gray-200">
                {order.items?.map((item, idx) => {
                  // ✅ Build variant text from backend (preferred)
                  const labelFromApi = item?.variant?.label;

                  // ✅ If label is not sent, build label from attributes object
                  ...
                  // ✅ Final variant text (best -> fallback)
                  const variantText =
                    labelFromApi ||
                    labelFromAttrs ||
                    item?.variant?.sku ||
                    item?.sku ||
                    (item?.variant_id ? `Variant #${item.variant_id}` : null);
                  ...
                  return (
                    ...
                    >
                        ...
                        <div>
                          ...
                          {variantText && (
                            <div className="text-xs text-slate-600 mt-1">
                              Variant: <span className="font-semibold">{variantText}</span>
                            </div>
                          )}
                      ...
                      <div className="font-extrabold">
                        {formatBDT(Number(item.price) * Number(item.qty))}
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="border border-gray-300 bg-white rounded-2xl p-6 soft-card">
              ...
              <div className="mt-4 border border-gray-300 rounded-xl p-4 bg-slate-50">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${paymentStatus === 'paid' || paymentStatus === 'completed' ? 'bg-emerald-100 text-emerald-700' :
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="flex flex-col items-center">
        <div className={`w-5 h-5 rounded-full flex items-center justify-center ${completed ? 'bg-emerald-600' : 'bg-gray-300'
          }`}>
